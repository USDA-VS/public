...
if [[ $1 == ab1 ]]; then
    ...
    python -u /home/tstuber/workspace/scripts/python_scripts/inputXLS.py | sed 's/ u//g' | tr "," "\t" | sed 's/\[//g' |sed 's/\]//g' |sed 's/ //g' | sed 's/^u//g' | sed 's/\.0//g' | tr -d "'"  > /home/shared/mycobacterium/bovis/scriptDependents/filterFile.txt
    ...
    genotypingcodes="/bioinfo11/TStuber/Results/_Brucella/bruc_tags.txt"
    # This file tells the script how to cluster VCFs
    DefiningSNPs="/bioinfo11/TStuber/Results//viruses/snp_analysis/pedv/script_dependents/pedv_Defining_SNPs.txt"
    FilterAllVCFs=no #(yes or no), Do you want to filter all VCFs?
    FilterGroups=no #(yes or no), Do you want to filter VCFs withing their groups, subgroups, and clades
    FilterDirectory="/Volumes/TStuber/Results/viruses/snp_analysis/pedv/script_dependents/FilterFiles" #Files containing positions to filter
    RemoveFromAnalysis="/bioinfo11/TStuber/Results/viruses/snp_analysis/pedv/script_dependents/RemoveFromAnalysis.txt"
    QUAL=300 # Minimum quality for calling a SNP
    lowEnd=1
    highEnd=350 # QUAL range to change ALT to N
    bioinfoVCF="/bioinfo11/TStuber/Results/viruses/snp_analysis/pedv/vcfs"
    echo "vcftofasta.sh ran as pedv"
    echo "Script vcftofasta.sh ran using pedv variables" > section5
...
clustalw2 -OUTFILE=alignment.txt -RANGE=1,2 -OUTPUT=FASTA -INFILE=${d}-fastaGroup.txt & 
/usr/local/bin/standard-RAxML-master/raxmlHPC-SSE3 -s ${d}-fastaGroup.txt -n ${d} -m GTRCAT -p 12345 &
wait
